...
function ProfileSectionEditorModal({ isOpen, sectionKey, itemIndex, profile, onClose, onSave, onReset }) {
  ...
  const submitSection = (event) => {
    ...
    if (sectionKey === 'summary' || sectionKey === 'languages' || sectionKey === 'skills') {
      if (sectionKey === 'languages') {
        onSave(sectionKey, {
        ...
        onSave(sectionKey, {
          skills: draft.skills.map((item) => item.value).filter((item) => item.trim().length > 0).join(', '),
        });
      } else {
              ...
              <>
                <div className="profile-editor-form__list">
                  {draft.skills?.map((item) => (
                    <div key={item.id} className="profile-editor-row">
                      <input className="profile-editor-form__input" value={item.value} onChange={(event) => updateArrayItem('skills', item.id, event.target.value)} />
                      <button type="button" className="profile-editor-row__remove" onClick={() => removeArrayItem('skills', item.id)}>
                        <Minus size={14} aria-hidden="true" />
                      </button>
                    </div>
                  ))}
                </div>
                <button type="button" className="profile-editor-form__add" onClick={() => addArrayItem('skills')}>
                ...
                {draft.experiences?.map((experience) => (
                  <div key={experience.id} className="profile-editor-experience-item">
                    ...
                    <div className="profile-editor-row profile-editor-row--three">
                      ...
                      <input className="profile-editor-form__input" value={experience.country} onChange={(event) => updateExperienceField(experience.id, 'country', event.target.value)} placeholder="Pais" />
                    </div>
