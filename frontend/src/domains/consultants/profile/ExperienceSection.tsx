...
function mapExperienceForCard(item: ExperienceRawItem, index: number): ExperienceCardItem {
  return {
...
interface ExperienceSectionProps {
  ...
  experiences: ExperienceRawItem[] | undefined;
}

function ExperienceSection({ showEdit, onAddItem, onEditItem, onRemoveItem, experiences }: ExperienceSectionProps) {
  const currentExperience = Array.isArray(experiences) && experiences.length > 0
    ? experiences.map(mapExperienceForCard)
    : [];

  // Si no hay experiencias y no esta en modo edicion, mostrar mensaje vacio
